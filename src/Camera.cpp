...
void Camera::Update(const float& deltaTime, GameObject& object) {
	...
	glm::vec2 pixelGridSize = LevelManager::GetInstance()->GetCurrentLevel()->GetPixelGridSize();
	position.x = glm::clamp(position.x, 0.0f, (pixelGridSize.x - (Engine_Pointer->windowGridSize * Engine_Pointer->tileSize).x));
	position.y = glm::clamp(position.y, 0.0f, (pixelGridSize.y - (Engine_Pointer->windowGridSize * Engine_Pointer->tileSize).y));
...
void Camera::FollowObject(const float& deltaTime, GameObject& object) {
	if (&object != nullptr) {
		if (isCameraCenter) {
			...
			glm::vec2 pixelGridSize = LevelManager::GetInstance()->GetCurrentLevel()->GetPixelGridSize();
			targetPosition.x = glm::clamp(targetPosition.x, 0.0f, (pixelGridSize.x - (Engine_Pointer->windowGridSize * Engine_Pointer->tileSize).x));
			targetPosition.y = glm::clamp(targetPosition.y, 0.0f, (pixelGridSize.y - (Engine_Pointer->windowGridSize * Engine_Pointer->tileSize).y));
